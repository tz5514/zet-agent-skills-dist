...
- **Dispatch channel selection:** select by runtime and delivery class. Codex, Cursor, and every other non-Claude-Code runtime use their native sub-agent facility. On Claude Code, standalone attempt-prompt operations whose own mechanical layer writes `dispatch_bootstrap` — currently `check-should-write-adr` and `check-adr-redundancy` — use the native `Agent` adapter below; quality-review modes retain their fixed headless CLI adapter, and supersession dispatch retains the CLI adapter defined by `SCAN-SUPERSESSION-PROMPT.md`. This is an exhaustive assignment, not a caller choice: no dispatch point may improvise a different channel or fallback. The rule governs only the channel and delivery adapter; which workflow steps dispatch at all remains with each operation.
- **HITL preflight authority bundle:** for `context_glossary_approval_preflight` only, first run `python3 scripts/preflight_authority_bundle.py prepare --runtime <codex|claude-code|cursor> --target-adr <target_adr_path> --run-dir <run_dir>`. The helper resolves `ADR-FORMAT.md` from the skill's own root and derives `CONTEXT.md` from the target path, assembles the reviewer prompt, and freezes the ordered roles `reviewer_prompt`, `adr_format`, `context`, and `target_adr` in one manifest by path, byte count, and SHA-256. Its single `emit` command rechecks the manifest and every authority before emitting one complete role-framed bundle. A changed manifest or authority, non-zero command, missing role marker, truncated bundle, or absent bundle-completion marker makes the review round `tool_failed`; never fall back to serial rereads. Pass the returned runtime adapter fields verbatim and do not add caller-written bootstrap text. After the round, use the returned `integrity_marker` and `review_mode` for the existing mechanical report validation and require the reviewer to echo `AUTHORITY_BUNDLE_COMPLETE: <authority_bundle_nonce>`. This changes only authority delivery: assembled prompt bytes and order, allowed inputs, reviewer duties, model, effort, foreground join, report generation, integrity-marker validation, and every semantic gate remain unchanged. Other review modes retain their ordinary prompt-delivery path.
- **Shared tier scope:** the runtime rows below (model, effort, channel, and foreground-join discipline) also govern the `check-should-write-adr` and `check-adr-redundancy` reviewer dispatches — each of those operations sets no dispatch policy of its own. Only dispatch policy is shared: `check-should-write-adr` prompt content, inputs, verdict, and report contract live in `CHECK-SHOULD-WRITE-ADR-PROMPT.md` and `scripts/check_should_write_adr.py`, with reply resolution through its own fixed-prefix path line (`CHECK_SHOULD_WRITE_ADR_REPORT_PATH:`) compared against the preassigned report path; `check-adr-redundancy` prompt content, inputs, verdict, and report contract live in `CHECK-ADR-REDUNDANCY-PROMPT.md` and `scripts/check_adr_redundancy.py`, with reply resolution through `CHECK_ADR_REDUNDANCY_REPORT_PATH:` compared against the preassigned evaluation-report path. Neither operation uses a per-run integrity marker or an authority bundle. The mode-specific delivery and validation sub-bullets below (authority bundle, marker echo, `REVIEW_REPORT_PATH:` capture) apply to quality-review modes only.
- **Runtime model:** select exactly one row by the prepared dispatch's `runtime` and apply only that row; do not interpret or execute instructions from any other runtime row. Prompt semantics and judgement rules are shared across runtimes; model assignment and effort are dispatch policy only.
  - **Codex only:** use `gpt-5.6-sol` with `xhigh` reasoning effort. Do not enable fast mode. Do not pass a `service_tier` override; record it as omitted/default in run evidence. Dispatch the reviewer through Codex's native sub-agent facility — never through any CLI call, including the codex CLI.
    - This row applies only when the selected runtime is Codex. Claude Code, Cursor, and every other runtime must skip this entire row without interpreting any Codex tool-surface instruction it contains. For the HITL preflight, pass the helper's `task_name`, `fork_turns`, `model`, `reasoning_effort`, and `message` fields to Codex `spawn_agent` verbatim and do not inherit the parent conversation. After `spawn_agent` returns, the main agent waits for that exact sub-agent to reach a terminal state and collects its result before continuing. The generated message's authority-acquisition phase uses one outer `functions.exec` with exactly one nested `tools.exec_command` call to the common bundle emitter; it validates command completion, exact output length, and bundle boundary markers before releasing any authority. That single-use invariant applies to authority acquisition, not to the gateway over the reviewer's whole lifetime. After validation, the authority-acquisition phase is closed. Later `functions.exec` gateway calls are permitted only to complete the acquired prompt's verdict/report contract: use nested `tools.apply_patch` to write the semantic verdict and nested `tools.exec_command` to run the mechanical report generator. Those later calls must not rerun the bundle emitter, must not retry authority acquisition, and must not read or reacquire any authority.
    - For `check-adr-redundancy`, call Codex `spawn_agent` with the attempt manifest's `dispatch_bootstrap` as the message and `fork_turns: "none"`; this reviewer does not inherit the parent conversation. Keep the row's model, reasoning effort, fast-mode, and service-tier settings unchanged, then wait for that exact sub-agent to reach a terminal state before resolving its reply. The operation uses no authority bundle.
  - **Cursor:** use `cursor-grok-4.6` with `xhigh` reasoning effort. Do not enable fast mode. Dispatch the reviewer through Cursor's native `Task` sub-agent facility — never through the Cursor agent CLI.
  ...
  - **Claude Code:** select one of the two adapters below from the operation's delivery class; never substitute one for the other.
    - **Standalone attempt-prompt operations:** dispatch `check-should-write-adr` and `check-adr-redundancy` through a native `Agent` sub-agent with `subagent_type: general-purpose`, `model: opus`, `run_in_background: false`, and the attempt manifest's `dispatch_bootstrap` passed verbatim as the prompt. Each call is foreground-synchronous: wait for the `Agent` terminal result before resolving the operation-specific path line. `check-adr-redundancy` must receive a fresh sub-agent context. These operations use their own prompt files and authority self-load contracts; do not use `claude -p`, do not use the authority-bundle helper, and do not execute a bundle `pipeline_command` for either operation. The native `Agent` surface has no per-invocation effort field, so do not move them to the CLI merely to set effort.
    - **Quality-review modes:** dispatch through the CLI using the helper's common bundle emitter as stdin to `claude -p --model opus --effort high`; execute the returned `pipeline_command` verbatim so `pipefail` rejects an emitter failure instead of starting a review with incomplete input. The complete reviewer prompt, ADR-FORMAT.md, CONTEXT.md, and target ADR therefore arrive before the first model turn, and the reviewer must not reread them. This is a **temporary patch**: in-harness Agent dispatch cannot set reasoning effort per invocation. **Exit conditions:** (i) when the platform lets in-harness sub-agent dispatch set per-invocation effort for these quality-review modes, return them to native sub-agent dispatch and remove this patch; (ii) if `claude -p` is excluded from subscription billing and becomes expensive, this patch is unusable and a replacement must be found.
    ...
    - Quality-review CLI completion is foreground-synchronous: the main agent waits for the `claude -p` process to exit and collects its terminal stdout and exit status before continuing. Never enable a background mode or detach the process.
    ...
    - Quality-review CLI output capture is unchanged: the reviewer's reply still carries the one line `REVIEW_REPORT_PATH: <path>`; the main agent extracts that line mechanically from the CLI stdout and reads the report file. The default `--output-format` (text) is sufficient. Standalone native `Agent` replies instead use their own `CHECK_SHOULD_WRITE_ADR_REPORT_PATH:` or `CHECK_ADR_REDUNDANCY_REPORT_PATH:` line. All three are resolved mechanically against their preassigned paths.
...
`terminal_result` carries only the terminals a current mode still produces, and the value set is closed: `quality_review` and `frozen_glossary_review` require `null`, while `context_glossary_approval_preflight` uses `null` or `blocked_by_structural_unreadability`. Any other value invalidates the round before report generation, rather than passing through as an unrecognized terminal.
