...
# modes whose prompt carries its terminal-stop instruction. The set is closed: a
# terminal outside it is an invalid verdict, not an unrecognized pass-through.
_TERMINAL_STOP_GATE = {
...
def expand_preflight_semantic_payload(payload, *, target_adr_path):
    ...
    terminal_result = STRUCTURAL_BLOCKED_TERMINAL if structural_terminal else None
    expanded = {
...
def validate_verdict_payload(payload, expected_integrity_marker):
    ...
    if terminal_result is not None:
        allowed_modes = _TERMINAL_ALLOWED_MODES.get(terminal_result)
        if allowed_modes is None:
            return False, "terminal_result_unknown"
...
def build_report(payload):
    ...
    # helper. That terminal is the only stop a current mode can reach, so every
    # other unevaluated gate is a mode-out gate taking the mode's skip reason.
    ...
    # Only the preflight's structural stop remains, and it is not a complete
    # review either way, so completion follows the mode alone.
    completed = review_mode == QUALITY_REVIEW_MODE
    return {
        ...
        "full_quality_review_notice": None if completed else _NOTICE[review_mode],
