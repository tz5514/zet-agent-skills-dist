...
4. **Delivery result handling — foreground, after the writes.** For ADR writes, consume `/adr produce-for-HITL`'s structured return — the same handoff envelope shape as `produce` (`draft_adr_path`, `structured_report_path`, `final_status`, `needs_user_ruling`), so no second parser is needed. Dispatch on `final_status`:
