...
`write` makes no ADR necessity judgment and has no necessity terminal. Whether the candidate merits an ADR is settled before `write`, in the calling orchestration: an agent flow holds a valid `check-should-write-adr` pass or the user's explicit, scope-named clearance, and a human-only entry carries the invoking user's own ruling. `write` trusts the caller — it neither receives nor validates authorization fields, and it never redoes, narrows, or overrules a completed `check-should-write-adr` ruling. One valid authorization also covers later revisions of the same draft that do not change the decisions' meaning, such as structural or wording repairs from a post-write check. Turning create-mode prose into atomic decisions is `write`'s work, but it must not add, replace, or expand decisions beyond the authorized candidate; substantively changing the reviewed decision scope stops the write and requires a new `check-should-write-adr` round on the new candidate input.
...
- For a new draft, derive `docs/adr/draft/` from the bounded context, call `generate_adr_id` in `scripts/adr_id.py` with that bounded context, and name the file `{returned id}-{slug}.md`. The writer chooses the slug and does not invent the id.
...
These two are the only terminal states; an execution failure surfaces as the failed operation itself, never as a third status value.
...
A new ADR filename is `{id}-{slug}.md`. Obtain a new `id` only by calling `generate_adr_id` in `scripts/adr_id.py` with the bounded context; the writer chooses the slug. ADR ids are **never reused** — once an id is taken it is never reclaimed, even after the original is archived. Historical four-digit ids and filenames remain valid for existing ADRs and for migration tooling.

## When to offer an ADR

Whether a candidate decision merits an ADR is judged by the ADR necessity conditions, before that candidate is written. Post-write ADR quality review is not an eligibility reviewer: it judges the written document, not whether the decision deserved an ADR. Their judgment semantics — the shared strict-evidence and verdict rules and every condition's positive-evidence and non-qualifying boundaries — live in exactly one place: `ADR-NECESSITY-CONDITIONS.md`. Load that document and judge by it. This section deliberately carries no condition text, no condition list, and no qualifying examples, so ADR-FORMAT.md can never drift into a second source of the necessity semantics.
