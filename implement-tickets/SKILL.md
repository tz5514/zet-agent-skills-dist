...
3. Use the current branch as the integration branch.
4. Read every ticket and extract:
   - identifier
   - title
   - status
   - `Blocked by`
   - parent spec reference, when present
5. Validate that identifiers are unique, every blocker exists, and the dependency graph has no cycles.
6. Check every ticket's triage status: only `ready-for-agent` tickets enter scheduling.
...
- **Bounded fix** — attributable to a single mechanical fix action, such as merging a duplicated implementation or unifying a divergent name: dispatch one fresh sub-agent to apply every bounded fix and commit the result.
...
- run the repository's full required checks
