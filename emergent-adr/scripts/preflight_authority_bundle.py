...
def _codex_message(spec, nonce):
    """Build the bootstrap that is valid only for a Codex native sub-agent."""
    encoded = json.dumps(spec, ensure_ascii=False, separators=(",", ":"))
    return f"""You are the independent ADR reviewer for `{REVIEW_MODE}`.

This bootstrap is only for the Codex runtime. It relies on the Codex-native `functions.exec` gateway and must not be dispatched to or interpreted by Claude Code, Cursor, or any other runtime.
