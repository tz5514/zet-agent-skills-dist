...
Dispatch is always **foreground-synchronous (blocking)** — never background. One sub-agent per scan packet is spawned in parallel as orchestration allows; the main agent joins all of them, structurally validates their output, performs final review, and returns structured `/adr` data for the consumer to render. The model tier is a landing parameter; moving it up or down requires re-passing the seeded tests.
