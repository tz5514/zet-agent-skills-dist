...
4. If preflight returns terminal `not_an_adr_candidate`, return `needs_user_ruling` with the validated necessity findings. This takes priority when the same outer report also contains glossary action data; retain that data for the consumer's next ruling turn.
5. Otherwise, if preflight reports a CONTEXT.md glossary approval need, stop and hand a `needs_context_ruling` user ruling back to the caller.
6. If preflight cannot evaluate because of structural unreadability or tool failure, stop with a non-pass terminal status.
7. If preflight completes without a blocking finding, return `hitl_preflight_passed` — a success state for the lightweight flow only, never the full `produce` `passed` status.
...
- A necessity ruling request has exactly `origin: "preflight"`, `terminal_result: "not_an_adr_candidate"`, and `findings`. It preserves all validated necessity findings in report order and removes only a fixed `action_data: null`; the outer preflight summary retains the complete raw blocking findings for consumer validation and same-session identity.
