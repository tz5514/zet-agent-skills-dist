...
3. **Links.** In conversation output, every file and folder path is a markdown hyperlink (workspace-relative). When pointing at a specific decision, attach a `#L` line anchor — the line number at output time; these anchors are point-in-time and going stale later is accepted. This linking rule applies **only to conversation output**: ADR prose rules are governed by `/adr` operations invoked through the Skill tool and are not restated here.
...
5. **Batch boundary.** Whichever of ✅ / 🔍 / 📣 a cycle actually emits is emitted once per documentation-gate cycle: when one cycle writes several files or triggers several scans, the results merge into a single block of each kind. At interview time the documentation gate takes the lightweight path and runs no scan, so it emits only 📝 → ✅ — 🔍 / 📣 do not appear then (see the 🔍 block's own note); the batch rule governs whichever blocks a cycle does emit.
...
7. **Block order within a gate cycle is fixed:** 📝 → ✅ → 🔍 → 📣 — at interview time only 📝 → ✅ appear, since the lightweight gate runs no scan.

## The ten blocks — zh-TW literal set (source version)
...
### 2. 📝 Bounded context 文件歸檔 — after every ratified answer
...
### 3. ✅ 交付檢查 — per gate cycle that wrote files, when foreground delivery checks pass

_Turn role: non-blocking notification block._

```
### ✅ 交付檢查
{交付結果清單，如「ADR {id}：輕量預檢通過（未跑完整品質審查與推翻掃描）；CONTEXT.md：自足性驗證通過」}
```

Heading and content stay on separate lines — the pass sentence is never merged into the H3 heading, keeping the visual structure consistent with every other block. ADR entries report the lightweight `produce-for-HITL` preflight pass — full ADR quality review and supersession scan have not run, so this is never worded as a completed `produce` acceptance; CONTEXT.md-only entries keep this skill's local self-sufficiency wording. The pass line's language follows the layering rules below. Rejected undefined-term candidates, if any, may be appended as a parenthetical note at the end of this block — see SKILL.md's documentation gate for the candidate-handling flow.

### 4. 🔍 ADR 決策推翻掃描 — emitted only when a supersession scan actually ran and reported; the interview-time documentation gate takes the lightweight path and runs no scan, so it does not emit this block (template retained for non-interview or future use)
...
### 5. 📣 ADR 決策推翻報告 — only when something was actually superseded, immediately after 🔍
...
### 6. 🚨 自足性驗證回報違規 — when the verifier reports a violation; the heading carries the re-review verdict
...
### 7. 🛑 自足性修正觸及決策內容 — hard rule: stop and ask
...
### 8. 🗺️ 待拍板議題 — one line before each question
...
### 9. 🎯 收尾前最後候選議題 — when you think you have no more questions
...
### 10. 🏁 收尾檢核 — at the end of each round
...
### 2. 📝 Bounded context documentation
...
```
### ✅ Delivery checks
{delivery result list, e.g. "ADR {id}: lightweight preflight passed (full quality review and supersession scan have not run); CONTEXT.md: self-sufficiency verification passed"}
```

### 4. 🔍 ADR supersession scan — not emitted at interview time (the lightweight gate runs no scan); template retained for non-interview or future use
...
### 6. 🚨 Self-sufficiency violation reported
...
### 7. 🛑 Self-sufficiency fix touches decision content
...
### 9. 🎯 Final candidate questions before closing
...
- **Machine-facing text is always English**, regardless of the environment language: the two sub-agent prompts (self-sufficiency verification and supersession scan) and the verdict tokens they emit (`PASS`, `VIOLATIONS`, `SCAN …: none`). Verdict tokens never switch with the environment language — they are machine-parse targets, not human interface.
