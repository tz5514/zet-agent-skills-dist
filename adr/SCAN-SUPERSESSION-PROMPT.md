...
Dispatch is always **foreground-synchronous (blocking)** for both CLI processes and native sub-agents. The main agent must wait for and collect every terminal result before it continues; receiving a dispatch handle is not completion. One call per scan packet may start in parallel as orchestration allows, but all calls remain behind a join barrier before structural validation, final review, or any later ADR lifecycle step. Never set a background mode or detach a dispatch. The model tier is a landing parameter; moving it up or down requires re-passing the seeded tests.
